...
class StyleTransfer:
    def __init__(self,content_image,style_image,style_weight=5,content_weight=0.025):
        ...
        model_path = os.path.join(BASE_DIR, 'vgg19-dcbb9e9d.pth')
        self.vgg19.load_state_dict(torch.load(model_path))
        self.img_ncols = 400
        self.img_nrows = 300
    ...
    def process_img(self,img_path):
        ...
        # Use os.path to correctly extract filename on both Windows and Unix
        img_name = os.path.basename(img_path)
        # Remove extension (handle both .jpg and .png, etc.)
        if '.' in img_name:
            img_name = img_name.rsplit('.', 1)[0]
        # Clean filename: remove invalid characters for Windows folder names
        invalid_chars = ['<', '>', ':', '"', '/', '\\', '|', '?', '*']
    ...
    def deprocess_img(self,x,index=None,return_img=False):
    ...
    def main_train(self,epoch=10,progress_callback=None,stop_check=None):
        ...
        cur,pre = 10,10
        no_improve_count = 0  # Count consecutive epochs without improvement
        min_epochs = min(10, epoch // 2)  # At least train for half of requested epochs or 10, whichever is smaller
        ...
        for i in range(1,epoch+1):
            ...
            if progress_callback:
                progress_callback(i, epoch, loss.item())
            
            # Improved early stopping: only stop if loss doesn't improve for multiple epochs
            # and we've trained for at least min_epochs
            if i > min_epochs:
                # Check if loss improved (with small tolerance for numerical precision)
                if cur < pre * 0.9999:  # Loss decreased by at least 0.01%
                    no_improve_count = 0
                else:
                    no_improve_count += 1
                    # Stop if loss hasn't improved for 5 consecutive epochs
                    if no_improve_count >= 5:
                        print(f'Early stopping! No improvement for {no_improve_count} epochs.')
                        break
            else:
                no_improve_count = 0  # Reset counter during minimum training period
                
            combination_param.data.clamp_(0,1)
            if i%5 == 0:
                # Fix: use combination_param instead of self.conbination_tensor
                self.deprocess_img(combination_param,i//5)
        
        # Return the final combination_param
